...
void saveEvs(int seed, int TupleNumber) {
  char filename[1024];
  sprintf(filename, "../dat/%dtuple_data_TupleNumber%d_seed%d_storage_c%d.dat",
...
int main(int argc, char* argv[]) {
  if (argc < 8 + 1) {
    fprintf(stderr,
            ...
            "<Exploratory> <Restart_Len> <UCB_C> <seed> \n");
    exit(1);
  }
  ...
  for (long long gid = 0; gid < MAX_GAMES; gid++) {
    ...
    while (true) {  // ゲームのループ
      ...
      if (gameOver(train_state)) {
        update(train_board, 0 - qs[train]);
        traincount++;
        if (traincount % STORAGE_FREQUENCY == 0) saveEvs(seed);
      } else {
        ...
        update(train_board, max_ev_r - qs[train]);
        traincount++;
        if (traincount % STORAGE_FREQUENCY == 0) saveEvs(seed);
      }
      ...
      for (int d = 0; d < 4; d++) {
        if (ns[d] == -1) continue;
        double ucb = (qs[d] + rs[d]) / max_q + UCB_C * sqrt(log(sum_n) / ns[d]);
        // printf("%d %f %f %f %f %f\n", d, ucb, qs[d], max_q, sum_n, ns[d]);
        if (max_ucb < ucb) {
          max_ucb = ucb;
          selected = d;
        }
      ...
      if (gameOver(state)) {
        ...
        if (turn - restart_start <= Restart_Len) {
          printf("restart end:x %d subgames until turns", restart_count);
